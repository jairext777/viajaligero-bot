...
import { escalateToHumanTool } from "./tools.js";
...
export async function runAgentTurn(opts: {
  ...
  for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
    ...
    if (response.stop_reason !== "tool_use") {
      return { replyText: extractText(response.content) };
    }
  ...
  return { replyText: "Uy, se me cruzaron los cables 😅 ya avisé al equipo para que te ayuden directamente." };
}
